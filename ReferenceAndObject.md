...
#### 举例:
简单的赋值语句 a = 10000, a 指向了 10000 所在的那块内存。a 引用的对象就是存放 1 的内存地址。is 判断是否是同一个对象，id()取得对象的内存地址  
...
输出是 True，a，b又是相同的对象了。这是因为在python启动的时候，常用的常数[-5, 256]就建立好，不销毁的对象，也就是a，b都引用相同的一块内存（存放10000的内存）,每次创建这个范围内的整数对象不会申请新的内存。
